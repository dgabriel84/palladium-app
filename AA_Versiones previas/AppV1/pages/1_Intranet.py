...
# -----------------------------------------------------------------------------
# VISUALIZACIÓN: RENDIMIENTO DEL MODELO (CURVAS + MATRIZ)
# -----------------------------------------------------------------------------
def rendimiento(probas_test, preds_test, y_test, recall=None, precision=None, f1scor=None, sobreajuste=None, acura=None, auc=None):
    """
    Genera un panel 2x2 con:
    - Matriz de confusión
    - Curva Precision-Recall
    - Curva ROC
    - Resumen de métricas

    Nota: en Streamlit devolvemos la figura (no hacemos plt.show()).
    """
    ...
    if acura is None:
        acura = accuracy_score(y_test, preds_test)
    if auc is None:
        try:
    ...
    if f1scor is None:
        f1scor = f1_score(y_test, preds_test, zero_division=0)

    fig, ax = plt.subplots(2, 2, figsize=(15, 12))
    plt.subplots_adjust(hspace=0.3, wspace=0.3)

    # --- Subplot 1: Matriz de Confusión ---
    palladium_cm = LinearSegmentedColormap.from_list(
        "palladium_cm",
        [COLOR_CREMA_CLARO, COLOR_DORADO],
    )
    cm = confusion_matrix(y_test, preds_test)
    disp_cm = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=["Cancelan", "No Cancelan"])
    disp_cm.plot(ax=ax[0, 0], cmap=palladium_cm, values_format="d", colorbar=False)
    ax[0, 0].set_title("Matriz de Confusión", color=COLOR_VERDE_OSCURO)

    # --- Subplot 2: Curva Precision-Recall ---
    try:
        ap_score = average_precision_score(y_test, probas_test)
        PrecisionRecallDisplay.from_predictions(y_test, probas_test, ax=ax[0, 1], color=COLOR_VERDE_OSCURO)
        ax[0, 1].set_title(f"Curva Precision-Recall (AP = {ap_score:.3f})", color=COLOR_VERDE_OSCURO)
    except Exception:
        ax[0, 1].set_title("Curva Precision-Recall (no disponible)", color=COLOR_VERDE_OSCURO)
        ax[0, 1].axis("off")

    # --- Subplot 3: Curva ROC ---
    try:
        roc_auc = roc_auc_score(y_test, probas_test)
        RocCurveDisplay.from_predictions(y_test, probas_test, ax=ax[1, 0], color=COLOR_VERDE_OSCURO)
        ax[1, 0].plot([0, 1], [0, 1], color=COLOR_DORADO, lw=2, linestyle="--", alpha=0.8)
        ax[1, 0].set_title(f"Curva ROC (AUC = {roc_auc:.3f})", color=COLOR_VERDE_OSCURO)
    except Exception:
        ax[1, 0].set_title("Curva ROC (no disponible)", color=COLOR_VERDE_OSCURO)
        ax[1, 0].axis("off")

    # --- Subplot 4: Resumen de Métricas ---
    sobreajuste_txt = f"{sobreajuste:.3f}" if isinstance(sobreajuste, (int, float)) else "—"
    stats_text = (
        "Métricas:\n\n"
        f"Accuracy: {acura:.3f}\n"
        f"AUC: {auc:.3f}\n"
        f"Recall (Detección): {recall:.3f}\n"
        f"Precision (Seguridad): {precision:.3f}\n"
        f"F1-Score: {f1scor:.3f}\n"
        f"Sobreajuste: {sobreajuste_txt}"
    )
    ax[1, 1].text(
        0.5,
        0.5,
        ...
        ha="center",
        va="center",
        bbox=dict(boxstyle="round", facecolor=COLOR_CREMA, edgecolor=COLOR_DORADO, alpha=0.85),
    )
    ax[1, 1].axis("off")
...
def main():
    ...
    if hasattr(st, "segmented_control"):
        try:
            selected_tab = st.segmented_control("", tabs, default=tabs[0], key="intranet_tab")
        except TypeError:
            selected_tab = st.segmented_control("", tabs)
    if not selected_tab:
        try:
            selected_tab = st.radio("", tabs, horizontal=True, key="intranet_tab", label_visibility="collapsed")
        except TypeError:
            selected_tab = st.radio("", tabs, horizontal=True, key="intranet_tab")
                ...
                try:
                    ...
                    if shap_available and model is not None:
                        try:
                            ...
                            # Colores corporativos Palladium (positivo = dorado, negativo = verde).
                            try:
                            ...
                            x = processed_df
                            if hasattr(model, "feature_name_"):
                                x = processed_df[model.feature_name_]

                            model_for_shap = model.booster_ if hasattr(model, "booster_") else model
                            ...
                            with st.spinner("Generando explicación SHAP..."):
                                ...
                                try:
                                    ...
                                    # Fallback: codificar categories como códigos numéricos.
                                    x_num = x.copy()
                                ...
                                st.session_state.shap_waterfall_label = st.session_state.get("last_manual_pred_label", "")
...
print(f"Probabilidad de cancelación: {{resultado:.2%}}")"""
        ...
        st.markdown("### Rendimiento del Modelo (Curvas y Matriz)")
        st.caption("Se calcula automáticamente con reservas que ya tienen `estado` (confirmada/cancelada).")

        base_dir = os.path.dirname(os.path.dirname(__file__))
        fuentes_perf = []

        web_path = os.path.join(base_dir, "reservas_web_2026.csv")
        if os.path.exists(web_path):
            try:
                df_web = pd.read_csv(web_path, on_bad_lines="skip")
                fuentes_perf.append(
                    pd.DataFrame(
                        {
                            "proba": pd.to_numeric(df_web.get("PROBABILIDAD_CANCELACION"), errors="coerce"),
                            "estado": df_web.get("ESTADO"),
                            "fuente": "web",
                        }
                    )
                )
        ...
        hist_path = os.path.join(base_dir, "data", "historial_reservas.csv")
        if os.path.exists(hist_path):
            try:
                ...
                fuentes_perf.append(
                    pd.DataFrame(
                        {
                            "proba": pd.to_numeric(df_hist.get("prob_cancelacion"), errors="coerce"),
                            "estado": df_hist.get("estado"),
                            "fuente": "historial",
                        }
                    )
                )
        ...
        if not fuentes_perf:
            st.info("No hay datos suficientes para calcular el rendimiento (no se han encontrado reservas con estado).")
        else:
            df_perf = pd.concat(fuentes_perf, ignore_index=True)
            df_perf = df_perf.dropna(subset=["proba", "estado"])

            # Normalizar probas a [0, 1] si vienen en %.
            df_perf["proba"] = df_perf["proba"].astype(float)
            mask_pct = df_perf["proba"] > 1.0
            if mask_pct.any():
                df_perf.loc[mask_pct, "proba"] = df_perf.loc[mask_pct, "proba"] / 100.0
            df_perf["proba"] = df_perf["proba"].clip(0.0, 1.0)

            estado_str = df_perf["estado"].astype(str).str.strip().str.lower()
            mask_cancel = estado_str.str.contains("cancel")
            mask_confirm = estado_str.str.contains("confirm")
            df_perf = df_perf[mask_cancel | mask_confirm].copy()
            estado_str = df_perf["estado"].astype(str).str.strip().str.lower()
            y_true = np.where(estado_str.str.contains("cancel"), 1, 0)

            # Solo se pueden calcular curvas si hay ambos casos.
            if len(set(y_true.tolist())) < 2:
                st.info("Aún no hay reservas canceladas registradas; se mostrarán las curvas cuando existan casos confirmados y cancelados.")
            else:
                threshold = st.slider("Umbral de clasificación", 0.0, 1.0, 0.35, 0.01, key="perf_threshold_prod")
                probas = df_perf["proba"].to_numpy()
                preds = (probas >= threshold).astype(int)
                fig = rendimiento(probas, preds, y_true)
    ...
    if selected_tab == "BUSCAR RESERVA":
        ...
        if buscar_reserva_por_id is None:
            ...
            if reserva:
                ...
                try:
                    r_id_tmp = reserva.get("id", reserva.get("id_reserva", ""))
                    ...
                    if pd.notna(llegada_val):
                        ...
                        # Generar SHAP (best-effort) para que sea visible sin depender de Predicción Manual.
                        shap_id = f"search:{r_id_tmp}"
                        if st.session_state.get("last_shap_source_id") != shap_id:
                            import importlib.util
                            ...
                            if shap_available and model is not None:
                                try:
                                    ...
                                    plt.close(fig)
                                    st.session_state.shap_waterfall_png = buf.getvalue()
                                    st.session_state.shap_waterfall_label = st.session_state.last_manual_pred_label
                                    st.session_state.last_shap_source_id = shap_id
                ...
                shap_label = st.session_state.get("shap_waterfall_label", "")
                if shap_png and shap_label == f"Reserva {r_id}":
                    st.markdown("### Explicabilidad (Waterfall SHAP)")
                    st.image(
                        shap_png,
...
print(f"Probabilidad de cancelación: {{resultado:.2%}}")"""
    ...
    if selected_tab == "SOBRE NOSOTROS":
        ...
        st.markdown("- [Jean Gustavo Cueva](https://www.linkedin.com/in/jean-gustavo-cueva-cajas-69074b90/)")

        st.markdown("---")
        st.markdown("**Integrantes del Equipo:**")
        st.markdown("- [Alejandro Batres](https://www.linkedin.com/in/alejandro-batres-3595909b/)")
        st.markdown("- [David Ostiz](https://www.linkedin.com/in/david-ostiz/)")
        st.markdown("- [Francisco Javier Barrionuevo](https://www.linkedin.com/in/franciscobarrionuevo/)")
        st.markdown("- [Gabriel Chamorro](https://www.linkedin.com/in/gabriel-chamorro-s%C3%A1nchez-aa4347a2/)")
        st.markdown("- [Jose Javier Martínez](https://www.linkedin.com/in/josejmartinezc/)")
    ...
    for accion in acciones:
        ...
        try:
            if funcion == "consultar_ocupacion":
                ...
                # Simulación de datos (en producción conectaría a DB)
                ocupacion = np.random.randint(65, 98)
                ventas = np.random.randint(150000, 500000)
                
                info = f"📊 **Ocupación {mes} {anio}**\n\n- Global: {ocupacion}%\n- Ventas: {ventas:,.0f}€\n- Tendencia: +{np.random.randint(2,10)}% vs año anterior"
                ...
                # Usar datos reales del session state si hay
                reservas = st.session_state.get('reservations', [])
                num_reservas = len(reservas)
                if num_reservas > 0:
                    probs = [r['cancel_prob'] for r in reservas]
                    avg_prob = sum(probs) / len(probs)
                    high_risk = sum(1 for p in probs if p > 0.6)
                else:
                    avg_prob = 0.15
                    high_risk = 0
                
                info = f"⚠️ **Análisis de Riesgo ({mes})**\n\n- Probabilidad Media Cancelación: {avg_prob:.1%}\n- Reservas en Riesgo Alto (>60%): {high_risk}\n- Acción recomendada: Contactar clientes VIP en riesgo."
                ...
                reservas = st.session_state.get('reservations', [])
                total_eur = sum(r['valor'] for r in reservas)
                total_pax = sum(r['pax'] for r in reservas)
                
                info = f"📈 **Resumen General**\n\n- Reservas Activas: {len(reservas)}\n- Revenue Total: {total_eur:,.0f}€\n- PAX Total: {total_pax}\n- Ocupación proyectada: 82%"
