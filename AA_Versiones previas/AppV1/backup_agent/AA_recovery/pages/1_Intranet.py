...
<style>
    ...
    /* Cabecera intranet */
    .intranet-header {{
        background: {COLOR_VERDE_CABECERA}; /* Color solicitado */
        padding: 1rem 2rem;
    ...
    .risk-high {{
        ...
        border-left: 4px solid {COLOR_RIESGO_ALTO};
    }}
    
    .risk-medium {{
        color: {COLOR_RIESGO_MEDIO};
        border-left: 4px solid {COLOR_RIESGO_MEDIO};
    }}
    
    .risk-low {{
        color: {COLOR_RIESGO_BAJO};
        border-left: 4px solid {COLOR_RIESGO_BAJO};
    }}
...
# Paises del CSV
PAISES = [
    "ESPANA", "ESTADOS UNIDOS", "CANADA", "MEXICO", "ALEMANIA", "REINO UNIDO",
...
# Fidelidad
PROGRAMAS_FIDELIDAD = [
    "Sin programa", "Palladium Rewards", "Palladium Connect", "WyndHam Rewards"
]
...
def main():
    ...
    with tab_prediccion:
        ...
        with col_form1:
            ...
            noches = st.number_input("Noches", min_value=1, value=7, step=1, key="pred_noches")
        ...
        if st.button("CALCULAR PROBABILIDAD", type="primary", use_container_width=True):
            with st.spinner("Procesando..."):
                try:
                    fecha_toma_combined = pd.to_datetime(fecha_toma_date)
                    ...
                    with col_r1:
                        ...
                        <div style="background: {color}; color: white; padding: 2rem; border-radius: 8px; text-align: center;">
                            ...
                            <p style="margin: 0.5rem 0; font-size: 3.5rem; font-weight: 300;">{cancel_prob:.0%}</p>
                    ...
                    with st.expander("Ver Features Procesadas"):
                ...
                with st.container():
                    ...
                    with col2:
                        ...
                        <div style="padding-left: 0.5rem;">
                            ...
                            <p style="margin: 0 0 0.2rem 0; font-size: 0.75rem; color: {COLOR_GRIS};">Llegada: {llegada_str} · {row.get('NOCHES',0)} noches</p>
                    ...
                    with col4:
                        st.markdown(f"""
                        <div style="background: {color}; color: white; padding: 0.3rem 0; border-radius: 4px; text-align: center; margin-top: 0.5rem;">
                            <p style="margin: 0; font-size: 0.85rem; font-weight: 600;">{prob*100:.0f}%</p>
                        </div>
                        """, unsafe_allow_html=True)
                    ...
                    with col5:
                        if prob >= 0.4:
                            ...
                            if st.button("RETENCIÓN", key=f"ret_{row.get('ID_RESERVA')}"):
                ...
                with col_debug:
                   ...
                   if raw_data:
                       with st.expander("Ver código para Notebook", expanded=False):
                           ...
                           if pd.isna(fidelidad_val) or fidelidad_val == 'nan' or fidelidad_val == 'None':
                               fidelidad_val = None
