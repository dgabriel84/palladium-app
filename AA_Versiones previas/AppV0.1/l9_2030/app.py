...
def calcular_coste_estancia(hotel_nombre, habitacion, llegada, noches, pax):
    ...
    """
    codigo_hotel = MAPEO_HOTELES_PRECIOS.get(hotel_nombre)
    if not codigo_hotel: return 200 * noches # Fallback
    ...
    for i in range(noches):
        ...
        if not precio_noche:
             precio_noche = 200.0 # Fallback 
             # O intentar buscar el mas cercano... por ahora fallback
        ...
        with col_btn:
            if st.button("CONFIRMAR RESERVA", type="primary", width="stretch", key="btn_confirm_final"):
                if not nombre or not email:
                    ...
                    st.session_state.cliente_fidelidad = "Palladium Rewards" if rewards_email else "Sin programa"
                    ...
                    # Avanzar a confirmación final real (o mostrarla aquí mismo)
                    st.session_state.wizard_step = 5 # Usamos paso 5 para la "Confirmación" final
                    st.rerun()
    ...
    for accion in acciones:
        ...
        try:
            if funcion == "seleccionar_destino":
                ...
                if hotel:
                    # Buscar el complejo de este hotel
                    complejo = obtener_complejo_de_hotel(hotel)
                    st.session_state.reserva_complejo = complejo
                    st.session_state.hotel_seleccionado_paso3 = hotel
                ...
                # Avanzar al paso 4 (Datos) si no estamos ya allí
                st.session_state.wizard_step = 4
...
def main():
    ...
    with st.sidebar:
        st.image("media/general/Logo.jpg", width="stretch")
        ...
        if prompt:
            ...
            with st.spinner("Pensando..."):
                ...
                historial_completo = st.session_state.chat_history[-10:] # Últimos 10
                historial_agente = [
