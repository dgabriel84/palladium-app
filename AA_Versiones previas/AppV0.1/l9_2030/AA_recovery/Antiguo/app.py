...
os.chdir(APP_DIR)  # FIX: Asegurar que CWD es el dir de la app para cargar imágenes relativas
CSV_RESERVAS = os.path.join(APP_DIR, "reservas_2026.csv")

# Importar agente conversacional V2 (con acciones)
try:
    from agent_v2 import chat_con_acciones, obtener_imagen_hotel, stt_groq_whisper as transcribir_audio, generar_audio_edge as tts_generator
    AGENT_AVAILABLE = True
except Exception as e:
    AGENT_AVAILABLE = False
    st.sidebar.error(f"Error cargando agente: {e}")

# Importar predictor de cancelaciones (modelo LightGBM real)
try:
    from predictor import predecir_cancelacion, explicar_reserva
    PREDICTOR_AVAILABLE = True
except Exception as e:
    PREDICTOR_AVAILABLE = False
    print(f"[PREDICTOR] No disponible: {e}")
...
# =============================================================================
# FUNCION UNIFICADA DE CONFIRMACION (BACKEND)
# =============================================================================
def crear_reserva_oficial(datos):
    """
    Crea una reserva oficial, la guarda en sesión y en CSV.
    Devuelve el número de reserva generado.
    """
    try:
        print(f"[DEBUG] crear_reserva_oficial llamado con: {datos}")
        
        # 1. Generar numero y timestamp
        c_compl = datos.get("complejo", "Riviera Maya")
        # Asegurar nombre correcto para el código
        if "costa" in c_compl.lower(): c_compl = "Costa Mujeres"
        elif "punta" in c_compl.lower(): c_compl = "Punta Cana"
        else: c_compl = "Riviera Maya"
        ...
        f_entrada = datos.get("fecha_entrada", datetime.now().date())
        if isinstance(f_entrada, str):
            try: f_entrada = datetime.strptime(f_entrada, "%Y-%m-%d").date()
            except: f_entrada = datetime.now().date()
        elif isinstance(f_entrada, datetime):
            f_entrada = f_entrada.date()
        datos["fecha_entrada"] = f_entrada
        
        f_salida = datos.get("fecha_salida", f_entrada + timedelta(days=7))
        if isinstance(f_salida, str):
            try: f_salida = datetime.strptime(f_salida, "%Y-%m-%d").date()
            except: f_salida = f_entrada + timedelta(days=7)
        elif isinstance(f_salida, datetime):
            f_salida = f_salida.date()
        datos["fecha_salida"] = f_salida
        
        # Calcular noches si no existe
        if "noches" not in datos or datos["noches"] is None or datos["noches"] <= 0:
            datos["noches"] = (f_salida - f_entrada).days
        
        # Asegurar campos requeridos para el tarjetón
        datos.setdefault("hotel", st.session_state.get("hotel_seleccionado", "Hotel"))
        datos.setdefault("habitacion", st.session_state.get("w_habitacion", "Habitación"))
        datos.setdefault("regimen", st.session_state.get("w_regimen", "All Inclusive"))
        datos.setdefault("tarifa", st.session_state.get("tarifa_seleccionada", "Flexible"))
        datos.setdefault("adultos", st.session_state.get("w_adultos", 2))
        datos.setdefault("ninos", st.session_state.get("w_ninos", 0))
        datos.setdefault("total_pax", datos["adultos"] + datos["ninos"])
        datos.setdefault("precio_total", 0.0)
        datos.setdefault("tiene_rewards", st.session_state.get("w_fidelizado", False))
        
        datos["lead_time"] = (f_entrada - datos["fecha_creacion"].date()).days
        
        # 2. Guardar en Session State
        if "reservas" not in st.session_state: st.session_state.reservas = {}
        st.session_state.reservas[numero] = datos.copy()  # Usar copia para evitar mutaciones
        st.session_state.reserva_confirmada = numero
        
        print(f"[DEBUG] Reserva guardada: {numero} -> {st.session_state.reservas[numero]}")
        
        # 3. Persistir en CSV
        try:
            # === PREDICCIÓN CON MODELO REAL (LightGBM) ===
            pais_cliente = st.session_state.get("w_pais", "ESPAÑA")
            
            if PREDICTOR_AVAILABLE:
                resultado_prediccion = predecir_cancelacion(
                    fecha_llegada=f_entrada,
                    noches=datos.get("noches", 7),
                    adultos=datos.get("adultos", 2),
                    ninos=datos.get("ninos", 0),
                    pais=pais_cliente,
                    es_rewards=datos.get("tiene_rewards", False),
                    nombre_hotel=datos.get("hotel", "Grand Palladium Colonial Resort & Spa"),
                    nombre_habitacion=datos.get("habitacion", "COL JUNIOR SUITE GV"),
                    valor_reserva=datos.get("precio_total", 1000.0)
                )
                
                if resultado_prediccion["success"]:
                    prob_cancelacion = resultado_prediccion["probabilidad"]
                    print(f"[PREDICTOR] Probabilidad real: {prob_cancelacion:.2%} (riesgo: {resultado_prediccion['riesgo']})")
                else:
                    # Fallback a simulación si hay error
                    print(f"[PREDICTOR] Fallback a simulación: {resultado_prediccion['error']}")
                    prob_base = 0.25
                    if datos["lead_time"] > 90: prob_base += 0.10
                    if datos.get("tiene_rewards", False): prob_base -= 0.10
                    if datos.get("noches", 1) >= 7: prob_base -= 0.05
                    prob_cancelacion = max(0.05, min(0.70, prob_base))
            else:
                # Fallback: Probabilidad simulada si el modelo no está disponible
                prob_base = 0.25
                if datos["lead_time"] > 90: prob_base += 0.10
                if datos.get("tiene_rewards", False): prob_base -= 0.10
                if datos.get("noches", 1) >= 7: prob_base -= 0.05
                prob_cancelacion = max(0.05, min(0.70, prob_base))
                print(f"[PREDICTOR] Usando simulación (modelo no disponible): {prob_cancelacion:.2%}")

            nueva_reserva_csv = {
                "ID_RESERVA": numero,
                "NOMBRE_HOTEL": datos.get("hotel", "Unknown"),
                "NOMBRE_HABITACION": datos.get("habitacion", "Unknown"),
                "LLEGADA": f_entrada.strftime("%Y-%m-%d"),
                "SALIDA": f_salida.strftime("%Y-%m-%d"),
                "FECHA_TOMA": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "NOCHES": datos.get("noches", 1),
                "ADULTOS": datos.get("adultos", 2),
                "PAX": datos.get("total_pax", 2),
                "VALOR_RESERVA": datos.get("precio_total", 0.0),
                "PROB_CANCELACION": prob_cancelacion,
                "CANCELADO": 0,
                "CANAL_CONSOLIDADO": datos.get("canal", "WEB_DIRECT"),
                "PAIS_TOP": pais_cliente,
                "TIENE_REWARDS": 1 if datos.get("tiene_rewards", False) else 0
            }
            ...
            print(f"Error persistencia CSV: {e}")
            
        print(f"[DEBUG] Reserva creada con éxito: {numero}")
        return numero
    except Exception as e:
...
def ejecutar_acciones(acciones):
    ...
    print(f"[DEBUG] ejecutar_acciones recibió: {acciones}")
    
    for accion in acciones:
        funcion = accion.get("funcion", "")
        params = accion.get("parametros", {})
        
        print(f"[DEBUG] Ejecutando función: {funcion} con parámetros: {params}")
        ...
        try:
            if funcion == "seleccionar_destino":
                ...
                # IMPORTANTE: Buscar y establecer destino/complejo automáticamente si no están definidos
                if not st.session_state.get("destino_seleccionado") or not st.session_state.get("complejo_seleccionado"):
                    # Buscar el hotel en DESTINOS para encontrar su destino y complejo
                    for pais_name, pais_data in DESTINOS.items():
                        for comp_name, comp_data in pais_data.get("complejos", {}).items():
                            for h_name in comp_data.get("hoteles", {}).keys():
                                if h_name.lower() == hotel.lower() or hotel.lower() in h_name.lower():
                                    st.session_state.destino_seleccionado = pais_name
                                    st.session_state.complejo_seleccionado = comp_name
                                    st.session_state.hotel_seleccionado = h_name  # Nombre exacto
                                    print(f"[DEBUG] Auto-detectado: {pais_name} > {comp_name} > {h_name}")
                                    break
                ...
                if hab:
                    # BUSQUEDA FUZZY DE HABITACION PARA EL WIDGET
                    # Recuperar las habitaciones del hotel seleccionado
                    try:
                        dest = st.session_state.get("destino_seleccionado", "Mexico")
                        comp = st.session_state.get("complejo_seleccionado", "Riviera Maya")
                        hot = st.session_state.get("hotel_seleccionado", "Grand Palladium Colonial Resort & Spa")
                        
                        # Buscar datos del hotel
                        hotel_data = DESTINOS.get(dest, {}).get("complejos", {}).get(comp, {}).get("hoteles", {}).get(hot, {})
                        if hotel_data:
                            # Buscar la options list real
                            habitaciones_keys = list(hotel_data.get("habitaciones", {}).keys())
                            
                            # Encontrar la mejor coincidencia
                            match = None
                            for h_key in habitaciones_keys:
                                if hab.lower() in h_key.lower() or h_key.lower() in hab.lower():
                                    match = h_key
                                    break
                            
                            if match:
                                st.session_state.w_habitacion = match
                            else:
                                # Fallback al primero si no encuentra
                                if habitaciones_keys:
                                    st.session_state.w_habitacion = habitaciones_keys[0]
                    except:
                        pass
                ...
                if hotel:
                    ...
                    # Inferir complejo y destino para que la UI cargue bien
                    if "costa" in hotel.lower():
                        st.session_state.complejo_seleccionado = "Costa Mujeres"
                        st.session_state.destino_seleccionado = "Mexico"
                    elif "punta" in hotel.lower() or "bavaro" in hotel.lower() or "turquesa" in hotel.lower():
                        st.session_state.complejo_seleccionado = "Punta Cana"
                        st.session_state.destino_seleccionado = "Republica Dominicana"
                    else:
                        st.session_state.complejo_seleccionado = "Riviera Maya"
                    ...
                    # Forzar vista paso 3 (Detalle hotel/habitaciones)
                    st.session_state.paso_actual = 3
                ...
                if tarifa:
                    # Mapeo a las keys reales de TARIFAS
                    for k in TARIFAS.keys():
                        if tarifa.lower() in k.lower():
                            st.session_state.tarifa_seleccionada = k
                            break
                ...
                st.session_state.w_fidelizado = bool(es_fid)
                if st.session_state.w_fidelizado:
                     st.session_state.chk_rewards = True # Sincronizar checkbox visual
                     st.session_state.chat_historial.append({"role": "assistant", "content": "✨ Marcado como cliente Palladium Rewards. ¡5% Dto aplicado!"})
            
            elif funcion == "confirmar_reserva":
                # Lógica de confirmación REAL y ROBUSTA (USANDO FUNCION UNIFICADA)
                try:
                    # 1. Recopilar datos sincronizados
                    c_compl = st.session_state.get("complejo_seleccionado", "Riviera Maya")
                    c_hotel = st.session_state.get("hotel_seleccionado", "Grand Palladium Colonial Resort & Spa")
                    
                    # 2. Obtener códigos exactos
                    hotel_code = "MAYA_COL"
                    for pais_data in DESTINOS.values():
                        for comp_name, comp_data in pais_data.get("complejos", {}).items():
                            for h_name, h_data in comp_data.get("hoteles", {}).items():
                                if h_name == c_hotel:
                                    hotel_code = h_data["hotel_code"]
                                    c_compl = comp_name # Normalizar nombre complejo
                                    break
                    
                    # 3. Datos de reserva desde WIDGETS
                    fecha_entrada = st.session_state.get("w_llegada", datetime.now().date())
                    fecha_salida = st.session_state.get("w_salida", datetime.now().date() + timedelta(days=7))
                    noches = (fecha_salida - fecha_entrada).days
                    adultos = st.session_state.get("w_adultos", 2)
                    ninos = st.session_state.get("w_ninos", 0)
                    total_pax = adultos + ninos
                    
                    habitacion = st.session_state.get("w_habitacion", "Junior Suite")
                    regimen = st.session_state.get("w_regimen", "All Inclusive")
                    tarifa_nombre = st.session_state.get("tarifa_seleccionada", "Flexible - Paga Ahora")

                    # 4. CALCULO DE PRECIO REAL
                    precio_total = 0.0
                    
                    precios_hotel = PRECIOS_ADR.get(hotel_code, {})
                    hab_key = list(precios_hotel.keys())[0] if precios_hotel else ""
                    if habitacion in precios_hotel:
                        hab_key = habitacion
                    else:
                        for k in precios_hotel.keys():
                            if k in habitacion or habitacion in k:
                                hab_key = k
                                break
                    
                    precios_habitacion = precios_hotel.get(hab_key, {})
                    
                    fecha_actual = fecha_entrada
                    pax_str = str(total_pax) 
                    if pax_str not in precios_habitacion and precios_habitacion:
                         pax_str = list(precios_habitacion.keys())[0]

                    precios_pax = precios_habitacion.get(pax_str, {})

                    for _ in range(noches):
                        iso = fecha_actual.isocalendar()
                        sem_key = f"{iso[0]}-{iso[1]:02d}"
                        adr = precios_pax.get(sem_key, 250.0)
                        precio_total += adr * total_pax
                        fecha_actual += timedelta(days=1)
                    
                    # Suplementos
                    precio_total += 80 * noches * total_pax
                    
                    # Descuentos
                    if "No Reembolsable" in tarifa_nombre: precio_total *= 0.85
                    elif "Paga Ahora" in tarifa_nombre: precio_total *= 0.95
                        
                    if st.session_state.get("w_fidelizado", False):
                        precio_total *= 0.95

                    # 5. CREAR RESERVA CON FUNCION UNIFICADA
                    datos_reserva = {
                        ...
                        "tiene_rewards": st.session_state.get("w_fidelizado", False),
                        "canal": "AGENTE_VIRTUAL"
                    }
                    
                    numero = crear_reserva_oficial(datos_reserva)
                    
                    # 6. CAMBIAR A PANTALLA CONFIRMACIÓN
                    st.session_state.paso_actual = 4
                    st.session_state.chat_historial.append({"role": "assistant", "content": f"✅ ¡Hecho! Tu reserva {numero} está confirmada. Aquí tienes tu tarjetón de confirmación."})
...
# -------------------------------------------------------------------------
def render_chat_v2():
    """Renderiza el chat del agente V2 en el contenedor actual con Voz."""
    if not AGENT_AVAILABLE:
        st.warning("Agente no disponible. Verifica agent_v2.py")
        return

    # Cabecera con Toggle de Voz
    col_head, col_voz = st.columns([3, 1])
    with col_head:
        st.markdown("### 💬 Sophie - Asistente")
        st.caption("Experta en Palladium Hotel Group")
    with col_voz:
        modo_voz = st.toggle("🎙️ Voz", key="modo_voz_active")

    # Contenedor de mensajes
    chat_container = st.container(height=450)
    with chat_container:
        if not st.session_state.chat_historial:
            st.info("👋 ¡Hola! Soy Sophie. Puedo ayudarte a encontrar el hotel perfecto o gestionar tu reserva. ¿Hablamos? ✨")
        
        for i, msg in enumerate(st.session_state.chat_historial):
            if msg["role"] == "user":
                st.markdown(f"**Tú:** {msg['content']}")
            else:
                st.markdown(f"**Sophie:** {msg['content']}")
    ...
    # --- LOGICA DE AUDIO INPUT / TEXT INPUT ---
    texto_a_enviar = None
    
    # 1. Entrada de Audio (Prioridad)
    if modo_voz:
        if hasattr(st, "audio_input"):
            audio_val = st.audio_input("Habla con Sophie", key="audio_input_widget")
            if audio_val:
                # Verificar si es un audio nuevo para no reprocesar lo mismo
                if "last_audio_id" not in st.session_state or st.session_state.last_audio_id != audio_val:
                    st.session_state.last_audio_id = audio_val
                    with st.spinner("Escuchando..."):
                        transcripcion = transcribir_audio(audio_val.getvalue())
                    if transcripcion:
                        texto_a_enviar = transcripcion
        else:
            st.warning("⚠️ Tu versión de Streamlit no soporta entrada de audio nativa. Actualiza a >=1.40")

    # 2. Entrada de Texto (si no hay audio procesado)
    if not texto_a_enviar:
        # Formulario para limpiar input al enviar
        with st.form(key="chat_form", clear_on_submit=True):
            col_in, col_btn = st.columns([4, 1])
            with col_in:
                user_text_input = st.text_input("Mensaje", placeholder="Escribe tu mensaje...", label_visibility="collapsed")
            with col_btn:
                submitted = st.form_submit_button("➤")
            
            if submitted and user_text_input:
                texto_a_enviar = user_text_input

    # Botón limpiar fuera del form
    if st.button("🗑️ Limpiar Chat", key="btn_limpiar_v2", use_container_width=True):
        st.session_state.chat_historial = []
        st.session_state.chat_imagenes = []
    ...
    if texto_a_enviar:
        ...
        st.session_state.chat_historial.append({"role": "user", "content": texto_a_enviar})
        ...
        estado_reserva = {
            ...
            "modo": st.session_state.get("modo_actual", "Reservas")
        }
        
        # Consultar Agente
        with st.spinner("Sophie está pensando..."):
        ...
        # Procesar respuesta
        print(f"[DEBUG] Agente devolvió: {resultado}")
        mensaje = resultado.get("mensaje", "No pude procesar tu mensaje.")
        acciones = resultado.get("acciones", [])
        imagen_hotel_key = resultado.get("imagen", None)
        
        # 1. Recuperar imagen si existe
        img_path = None
        if imagen_hotel_key:
            img_path = obtener_imagen_hotel(imagen_hotel_key)
        
        # 2. Guardar respuesta en historial
        st.session_state.chat_historial.append({"role": "assistant", "content": mensaje})
        st.session_state.chat_imagenes.append(img_path)
        
        # 3. Marcar para reproducir audio en la siguiente recarga
        st.session_state.play_audio_next_run = True

        # 4. Ejecutar acciones (esto puede provocar un rerun)
        if acciones:
            ejecutar_acciones(acciones)
            
        st.rerun()

    # --- VOICE OUTPUT (Native st.audio) ---
    # Leer el último mensaje si es del asistente y está activo el modo voz
    should_play = st.session_state.get("play_audio_next_run", False)
    modo_voz_activo = st.session_state.get("modo_voz_active", False)
    
    print(f"[DEBUG TTS] should_play={should_play}, modo_voz_activo={modo_voz_activo}, historial_len={len(st.session_state.chat_historial) if st.session_state.chat_historial else 0}")
    
    if modo_voz_activo and should_play and st.session_state.chat_historial:
        ...
        if last_msg["role"] == "assistant":
            ...
            print(f"[DEBUG TTS] Generando audio para: {clean_text[:50]}...")
            
            # Generar audio
            audio_fp = tts_generator(clean_text)
            
            if audio_fp:
                # Reproductor nativo (permite pausar)
                st.audio(audio_fp, format='audio/mp3', autoplay=True)
                print("[DEBUG TTS] Audio generado y reproduciendo")
            else:
                print("[DEBUG TTS] ERROR: audio_fp es None")
                
        # Resetear flag para no repetir audio infinitamente
        st.session_state.play_audio_next_run = False
...
with col_nav2:
    area = st.radio("Navegación", ["Reservas", "Intranet"], horizontal=True, label_visibility="collapsed")
...
with col_contenido:
    ...
    if area == "Reservas":
        ...
        # -------------------------------------------------------------------------
            ...
            # Protección: si destino es None, volver al paso 1
            if not destino or destino not in DESTINOS:
                st.warning("Por favor, seleccione primero un destino.")
                st.session_state.paso_actual = 1
                st.rerun()
        ...
        # -------------------------------------------------------------------------
            ...
            # PROTECCIÓN ROBUSTA: Verificar que todos los datos necesarios existen
            if not destino or destino not in DESTINOS:
                st.warning("Por favor, seleccione primero un destino.")
                st.session_state.paso_actual = 1
                st.rerun()
            
            if not complejo or complejo not in DESTINOS.get(destino, {}).get("complejos", {}):
                st.warning("Por favor, seleccione primero un complejo.")
                st.session_state.paso_actual = 2
                st.rerun()
            ...
            with col1:
                ...
                regimen = st.radio("Régimen", list(REGIMENES.keys()), label_visibility="collapsed", key="w_regimen")
            ...
            with col_confirmar:
                ...
                def procesar_confirmacion(datos):
                    # USAR FUNCION UNIFICADA
                    numero = crear_reserva_oficial(datos)
                    # Cambiar paso
                    st.session_state.paso_actual = 4
                    st.session_state.reserva_confirmada = numero

                if st.button("CONFIRMAR RESERVA", key="btn_confirmar_final", type="primary", use_container_width=True):
                    procesar_confirmacion(datos_reserva_final)
                    st.rerun()
        ...
        # -------------------------------------------------------------------------
            ...
            if numero is None or numero not in st.session_state.reservas:
                ...
                print(f"[DEBUG] Paso 4: numero={numero}, reservas keys={list(st.session_state.reservas.keys()) if hasattr(st.session_state, 'reservas') else 'N/A'}")
                st.session_state.paso_actual = 1
                st.rerun()
        
            reserva = st.session_state.reservas[numero]
            print(f"[DEBUG] Mostrando tarjetón para reserva: {reserva}")
            
            # Formatear fechas de forma segura
            try:
                if isinstance(reserva['fecha_entrada'], (datetime, date)):
                    fecha_entrada_str = reserva['fecha_entrada'].strftime('%d/%m/%Y')
                else:
                    fecha_entrada_str = str(reserva['fecha_entrada'])
                    
                if isinstance(reserva['fecha_salida'], (datetime, date)):
                    fecha_salida_str = reserva['fecha_salida'].strftime('%d/%m/%Y')
                else:
                    fecha_salida_str = str(reserva['fecha_salida'])
            except Exception as e:
                print(f"[DEBUG] Error formateando fechas: {e}")
                fecha_entrada_str = str(reserva.get('fecha_entrada', 'N/A'))
                fecha_salida_str = str(reserva.get('fecha_salida', 'N/A'))

            # Obtener valores con fallbacks seguros
            hotel_name = reserva.get('hotel', 'Hotel no especificado')
            habitacion_name = reserva.get('habitacion', 'Habitación estándar')
            regimen_name = reserva.get('regimen', 'All Inclusive')
            noches = reserva.get('noches', 1)
            adultos = reserva.get('adultos', 2)
            ninos = reserva.get('ninos', 0)
            precio_total = reserva.get('precio_total', 0)
            tarifa_name = reserva.get('tarifa', 'Flexible')
            ...
            <div class="reserva-confirmada">
                ...
                <p>{fecha_entrada_str} - {fecha_salida_str} | {noches} noches</p>
                <p>{habitacion_name} | {regimen_name}</p>
                <p>{adultos} adultos{f", {ninos} niños" if ninos > 0 else ""}</p>
                <div class="precio-final">${precio_total:,.0f} USD</div>
                <p style="opacity:0.7">Tarifa: {tarifa_name}</p>
            </div>
            """, unsafe_allow_html=True)
            ...
            if st.button("BUSCAR"):
                if numero_busqueda in st.session_state.reservas:
                    ...
                    with col1:
                        ...
                        st.write(f"**Hotel:** {r.get('hotel', 'N/A')}")
                        
                        # Formatear fechas de forma segura
                        fe = r.get('fecha_entrada')
                        fs = r.get('fecha_salida')
                        fe_str = fe.strftime('%d/%m/%Y') if isinstance(fe, (datetime, date)) else str(fe)
                        fs_str = fs.strftime('%d/%m/%Y') if isinstance(fs, (datetime, date)) else str(fs)
                        st.write(f"**Fechas:** {fe_str} - {fs_str}")
                        
                        st.write(f"**Habitacion:** {r.get('habitacion', 'N/A')} | {r.get('regimen', 'N/A')}")
                        st.write(f"**Huéspedes:** {r.get('total_pax', r.get('adultos', 0) + r.get('ninos', 0))} personas")
                        st.write(f"**Tarifa:** {r.get('tarifa', 'N/A')}")
                        st.write(f"**Total:** ${r.get('precio_total', 0):,.0f} USD")
        ...
        with tab2:
            ...
            if df_reservas.empty:
                ...
                if len(df_filtrado) == 0:
                    ...
                    st.markdown("---")
                    st.markdown("#### 🔍 Analizar reserva con IA")
                
                    id_seleccionado = st.text_input(
                        "Introduce ID de reserva para análisis detallado",
                        placeholder="Ej: 610000009601",
                        key="id_reserva_analisis"
                    )
                
                    if id_seleccionado:
                        # Buscar en df_filtrado primero
                        reserva_sel = df_filtrado[df_filtrado['ID_RESERVA'].astype(str) == id_seleccionado]
                        
                        # Si no se encuentra, buscar en todo el DataFrame
                        if reserva_sel.empty and "df_reservas" in st.session_state:
                            reserva_sel = st.session_state.df_reservas[
                                st.session_state.df_reservas['ID_RESERVA'].astype(str) == id_seleccionado
                            ]
                        
                        # Si aún no se encuentra, recargar CSV
                        if reserva_sel.empty:
                            try:
                                df_reload = pd.read_csv(CSV_RESERVAS)
                                reserva_sel = df_reload[df_reload['ID_RESERVA'].astype(str) == id_seleccionado]
                            except:
                                pass
                        
                        if not reserva_sel.empty:
                            r = reserva_sel.iloc[0].to_dict()
                            prob = r.get('PROB_CANCELACION', 0)
                        
                            col_d1, col_d2 = st.columns([2, 1])
                            with col_d1:
                                st.write(f"**Hotel:** {r.get('NOMBRE_HOTEL', 'N/A')}")
                                st.write(f"**Habitación:** {r.get('NOMBRE_HABITACION', 'N/A')}")
                                llegada = r.get('LLEGADA', '')
                                salida = r.get('SALIDA', '')
                                if hasattr(llegada, 'strftime'):
                                    llegada = llegada.strftime('%d/%m/%Y')
                                if hasattr(salida, 'strftime'):
                                    salida = salida.strftime('%d/%m/%Y')
                                st.write(f"**Fechas:** {llegada} - {salida} ({r.get('NOCHES', 'N/A')} noches)")
                                st.write(f"**Huéspedes:** {r.get('PAX', 2)} personas ({r.get('ADULTOS', 2)} adultos)")
                                st.write(f"**Canal:** {r.get('CANAL_CONSOLIDADO', 'N/A')} | País: {r.get('PAIS_TOP', 'N/A')}")
                                st.write(f"**Valor:** ${r.get('VALOR_RESERVA', 0):,.0f}")
                            ...
                            st.caption("Importancia de cada variable en la decisión del modelo")
                            
                            if PREDICTOR_AVAILABLE:
                                with st.spinner("Analizando factores de riesgo..."):
                                    # Convertir a dict para pasar a la funcion
                                    reserva_sel_dict = reserva_sel.iloc[0].to_dict()
                                    explicacion = explicar_reserva(reserva_sel_dict)
                                    
                                    if explicacion["success"]:
                                        feature_values = explicacion.get("feature_values", {})
                                        
                                        # Crear DataFrame para Plotly
                                        chart_data = []
                                        
                                        # Nombres más amigables para las variables
                                        nombres_amigables = {
                                            "LEAD_TIME": "Anticipación (días)",
                                            "ADR": "Precio por noche",
                                            "REV_PAX": "Ingreso por huésped",
                                            "NOCHES": "Duración estancia",
                                            "ADULTOS": "Nº Adultos",
                                            "PALLADIUM_REWARDS": "Es cliente Rewards",
                                            "TIENE_FIDELIDAD": "Tiene fidelización",
                                            "TIPO_VIAJERO": "Tipo viajero",
                                            "ES_GRUPO": "Es grupo",
                                            "MES_LLEGADA_SIN": "Mes llegada (cíclico)",
                                            "MES_LLEGADA_COS": "Mes llegada (cíclico)",
                                            "HORA_TOMA_SIN": "Hora reserva (cíclico)",
                                            "HORA_TOMA_COS": "Hora reserva (cíclico)",
                                            "PAIS_TOP_200": "País cliente",
                                            "FUENTE_NEGOCIO_SEGMENTO_CLIENTE": "Canal/Segmento",
                                            "COMPLEJO_RESERVA": "Complejo+Habitación",
                                            "HOTEL_HABITACION_TOP": "Tipo habitación"
                                        }
                                        for key, data in importancias.items():
                                            if data['importancia'] > 0:
                                                nombre = nombres_amigables.get(key, key.replace("_", " ").title())
                                                valor = data["valor"]
                                                if isinstance(valor, (int, float)):
                                                    if key == "ADR" or key == "REV_PAX":
                                                        valor = f"${valor:,.0f}"
                                                    else:
                                                        valor = f"{valor:.2f}"
                                                chart_data.append({
                                                    "Variable": f"{nombre}",
                                                    "Importancia": data.get("importancia", 0),
                                                    "Valor": str(valor)[:30]  # Limitar longitud
                                                })
                                        ...
                                        if chart_data:
                                            ...
                                            # Mostrar valores de features más importantes
                                            st.markdown("**Valores clave de esta reserva:**")
                                            col_f1, col_f2, col_f3 = st.columns(3)
                                            with col_f1:
                                                lead = feature_values.get("LEAD_TIME", "N/A")
                                                st.metric("Anticipación", f"{lead} días")
                                            with col_f2:
                                                adr = feature_values.get("ADR", 0)
                                                st.metric("ADR", f"${adr:,.0f}" if isinstance(adr, (int, float)) else adr)
                                            with col_f3:
                                                rewards = "Sí" if feature_values.get("PALLADIUM_REWARDS", 0) else "No"
                                                st.metric("Palladium Rewards", rewards)
                                        else:
                                            st.info("No hay factores de riesgo significativos que mostrar.")
                                            
                                    else:
                                        st.warning(f"No se pudo generar explicación: {explicacion.get('error', 'Error desconocido')}")
                            else:
                                st.info("Predictor ML no disponible. Usando probabilidad almacenada.")
                        
                            # Botones de acción
                            st.markdown("---")
                            st.markdown("##### 🎯 Acciones de retención")
                            col_a1, col_a2, col_a3 = st.columns(3)
                            with col_a1:
                                st.button("📧 Enviar oferta retención", key="btn_oferta", disabled=True)
                            with col_a2:
                                st.button("⬆️ Upgrade habitación", key="btn_upgrade", disabled=True)
                            with col_a3:
                                st.button("📞 Contactar cliente", key="btn_contactar", disabled=True)
                        else:
                            st.warning(f"Reserva '{id_seleccionado}' no encontrada. Verifica el ID.")
