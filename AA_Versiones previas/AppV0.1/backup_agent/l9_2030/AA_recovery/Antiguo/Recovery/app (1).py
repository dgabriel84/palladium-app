...
with col_contenido:
    ...
    if area == "Reservas":
        ...
        # -------------------------------------------------------------------------
                ...
                with col_c:
                    if datos_hotel.get("solo_adultos", False):
                        cunas = 0
                         ...
                         cunas = st.number_input("Cunas (0-2)", min_value=0, max_value=2, value=st.session_state.w_cunas, key="w_cunas")
            ...
            # Botón Hazte Miembro (simulado visualmente)
            col_rew_1, col_rew_2, col_rew_3 = st.columns([1, 2, 1])
            with col_rew_2:
                # Usar key unica
                if "w_fidelizado" not in st.session_state: st.session_state.w_fidelizado = False
                
                label_btn = "HAZTE MIEMBRO" if not st.session_state.w_fidelizado else "MIEMBRO ACTIVO ✓"
                type_btn = "secondary" if not st.session_state.w_fidelizado else "primary"
                
                if st.button(label_btn, key="btn_rewards", use_container_width=True, type=type_btn):
                    st.session_state.w_fidelizado = not st.session_state.w_fidelizado
                    st.rerun()
            ...
            opciones_tarifa = list(TARIFAS.keys())
            
            # Formatear opciones para mostrar descuento
            def format_tarifa(nombre):
                t = TARIFAS[nombre]
                desc = f" (-{int(t['descuento']*100)}%)" if t['descuento'] > 0 else ""
                return f"{nombre}{desc} | {t['descripcion']}"

            tarifa = st.radio(
                "Elige tu tarifa:",
                opciones_tarifa,
                index=opciones_tarifa.index(st.session_state.tarifa_seleccionada) if st.session_state.tarifa_seleccionada in opciones_tarifa else 0,
                format_func=format_tarifa,
                key="radio_tarifa"
            )
