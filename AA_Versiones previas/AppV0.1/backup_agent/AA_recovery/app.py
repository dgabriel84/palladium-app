...
<style>
    ...
    .risk-high {{ color: {COLOR_RIESGO_ALTO}; border-left: 4px solid {COLOR_RIESGO_ALTO}; }}
    .risk-medium {{ color: {COLOR_RIESGO_MEDIO}; border-left: 4px solid {COLOR_RIESGO_MEDIO}; }}
    .risk-low {{ color: {COLOR_RIESGO_BAJO}; border-left: 4px solid {COLOR_RIESGO_BAJO}; }}
    ...
    .stButton > button:hover {{ background: {COLOR_DORADO_OSCURO}; }}
...
PROGRAMAS_FIDELIDAD = ["Sin programa", "Palladium Rewards", "Palladium Connect"]
...
def cargar_reservas_csv() -> pd.DataFrame:
    ...
    if os.path.exists(RESERVAS_WEB_PATH):
        try:
            df_web = pd.read_csv(RESERVAS_WEB_PATH)
            # Limpieza básica web
            ...
            st.error(f"Error leyendo reservas web: {e}")
