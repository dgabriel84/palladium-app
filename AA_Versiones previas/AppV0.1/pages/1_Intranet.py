...
def main():
    ...
    # MAIN LAYOUT
    # -------------------------------------------------------------------------
    st.markdown('<div class="intranet-header"><h2>Panel de Inteligencia de Negocio</h2></div>', unsafe_allow_html=True)
...
print(f"Probabilidad de cancelación: {{resultado:.2%}}")"""
    ...
    # -------------------------------------------------------------------------
    # PIE DE PAGINA (Con Transparencia)
    # -------------------------------------------------------------------------
    st.markdown("---")
    ...
    if os.path.exists(pie_path):
        ...
        st.markdown(
            ...
            <div style="display: flex; justify-content: center; margin-top: 20px; margin-bottom: 20px;">
                <img src="data:image/png;base64,{encoded}" style="width: 100%; max-width: 100%; opacity: 0.3; transition: opacity 0.3s;">
            </div>
            """, 
            unsafe_allow_html=True
        )
    ...
    for accion in acciones:
        ...
        try:
            if funcion == "consultar_ocupacion":
                mes = params.get("mes", "junio").lower()
                ...
                if df.empty:
                    info = "⚠️ No hay datos históricos disponibles para analizar ocupación."
                else:
                    # Filtro basico mes/año
                    if not pd.api.types.is_datetime64_any_dtype(df['LLEGADA']):
                        df['LLEGADA'] = pd.to_datetime(df['LLEGADA'], errors='coerce')
                    ...
                    if mes_num:
                        ...
                        df_filt = df[(df['LLEGADA'].dt.month == mes_num) & (df['LLEGADA'].dt.year == anio)]
                    else:
                        df_filt = df
                    
                    # Calcular Ocupación Real vs Capacidad Total (Aproximada global 1500 habs si no se especifica hotel)
                    # En una implementación real sumaría capacidades de todos los hoteles
                    capacidad_total_diaria = 1500 
                    dias_mes = 30 # Simplificación
                    capacidad_mes = capacidad_total_diaria * dias_mes
                    
                    # Noches totales vendidas (aprox)
                    noches_vendidas = df_filt['NOCHES'].sum() if not df_filt.empty else 0
                    if pd.isna(noches_vendidas): noches_vendidas = 0
                    
                    pct_ocupacion = (noches_vendidas / capacidad_mes) if capacidad_mes > 0 else 0
                    
                    # Overbooking (si > 100%)
                    pct_overbooking = max(0, pct_ocupacion - 1.0)
                    
                    ventas = df_filt['VALOR_RESERVA'].sum() if not df_filt.empty else 0
                    
                    info = f"""📊 **Análisis - {mes.capitalize()} {anio}**
                    
| Métrica | Valor | Notas |
| :--- | :--- | :--- |
| **Ocupación Est.** | `{pct_ocupacion:.1%}` | Sobre capacidad teórica |
| **Overbooking** | `{pct_overbooking:.1%}` | {'⚠️ Crítico' if pct_overbooking > 0 else '✅ Ok'} |
| **Ventas Totales** | `{ventas:,.0f}€` | Revenue bruto mes |
| **Noches Vendidas**| `{noches_vendidas:,.0f}` | Total room-nights |

*Nota: Cálculo basado en capacidad teórica estática.*"""
                ...
                modo = params.get("modo", "mayor_riesgo") # o "menor_riesgo"
                
                # 1. Obtener DataFrame (Prioridad Session State > Maestro)
                df = pd.DataFrame()
                if 'df_maestro' in st.session_state:
                    df = st.session_state['df_maestro'].copy()
                
                if df.empty:
                     info = "⚠️ No hay datos históricos disponibles para analizar."
                else:
                    # 2. Filtrar por Mes
                    if not pd.api.types.is_datetime64_any_dtype(df['LLEGADA']):
                        df['LLEGADA'] = pd.to_datetime(df['LLEGADA'], errors='coerce')
                    ...
                    if mes_num:
                        ...
                        df_mes = df # Si no reconoce mes, todo
                    
                    # 3. Calcular Estadísticas
                    total_reservas = len(df_mes)
                    if total_reservas > 0:
                        prob_media = df_mes['PROBABILIDAD_CANCELACION'].mean()
                        riesgo_alto = len(df_mes[df_mes['PROBABILIDAD_CANCELACION'] > 0.6])
                        
                        # 4. Top N Riesgo (Mayor o Menor)
                        ascending = (modo == "menor_riesgo")
                        top_riesgo = df_mes.sort_values('PROBABILIDAD_CANCELACION', ascending=ascending).head(top_n)
                        
                        label_modo = "MENOR" if ascending else "MAYOR"
                        
                        listado_txt = ""
                        for _, row in top_riesgo.iterrows():
                            # Asegurar campos existen
                            cliente = row.get('CLIENTE', 'Cliente')
                            hotel = row.get('NOMBRE_HOTEL_REAL', 'Hotel')
                            prob = row.get('PROBABILIDAD_CANCELACION', 0)
                            id_res = row.get('ID_RESERVA', 'ID')
                            
                            # Formato Markdown Table Row
                            hotel_short = (hotel[:15] + '..') if len(hotel) > 15 else hotel
                            listado_txt += f"| {id_res} | {cliente} | {hotel_short} | {prob:.1%} |\n"
                        
                        info = f"""⚠️ **Análisis de Riesgo ({mes.capitalize()})**

- **Probabilidad Media del Mes:** {prob_media:.1%}
- **Reservas en Riesgo Alto (>60%):** {riesgo_alto} de {total_reservas}

**TOP {top_n} {label_modo} RIESGO:**

| ID Reserva | Cliente | Hotel | Riesgo |
| :--- | :--- | :--- | :--- |
{listado_txt}

**Acción recomendada:** {'Fidelizar a estos clientes seguros.' if ascending else 'Revisar detalle de estas reservas prioritarias.'}"""
                    else:
                        info = f"ℹ️ No hay reservas registradas para el mes de **{mes}**."
...
def render_sidebar_agent():
    ...
    with st.sidebar:
        ...
        <style>
            /* Transparencia para la imagen del logo en el sidebar */
            [data-testid="stSidebar"] img {
                opacity: 0.8; /* Ajusta este valor (0.0 a 1.0) */
